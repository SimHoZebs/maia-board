...
// interactive form.
export function CandidateRow({ index, san, metric, delta, isPlayed, preview }: {
  index: number; san: string; metric: string; delta?: string; isPlayed: boolean;
  preview?: { label: string; active: boolean; onPreview: () => void; onClear?: () => void; onSelect: () => void };
}) {
  ...
  return <li className={isPlayed ? 'played' : undefined}><span className="rank">{index + 1}</span>{preview ?
    <button type="button" className="candidate-reading" aria-label={preview.label} aria-pressed={preview.active} onMouseEnter={preview.onPreview} onFocus={preview.onPreview} onMouseLeave={preview.onClear} onBlur={preview.onClear} onClick={preview.onSelect}>{inner}</button> :
