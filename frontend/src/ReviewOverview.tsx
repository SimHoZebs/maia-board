...
export function ReviewOverview({ review, ply, userSide, branch, onInspect }: {
  ...
  const summary = summarizeReview(review.nodes, review.qualities);
  ...
  return <section className="review-overview" aria-labelledby="overview-title">
    <div className="overview-heading"><h2 id="overview-title">{branch ? 'Explored line overview' : 'Game overview'}</h2>
      {summary.total > 0 && <span className="review-coverage">{summary.reviewed} / {summary.total} moves reviewed</span>}
    </div>
    {!summary.total ? <p className="empty-copy">Play or load some moves to see an accuracy summary.</p> : <>
      <ReviewCharts review={review} ply={ply} sans={review.nodes.slice(1).map((node, index) => candidateSan(review.nodes[index].fen, node.moves[index]))} onView={onInspect} />
      <div className="accuracy-summary">
        {summary.sides.map(side => <section key={side.color} className={`accuracy-card${userSide === side.color ? ' own-side' : ''}`} aria-label={`${sideName(side.color)} accuracy`}>
          ...
          <span className="accuracy-coverage">{side.reviewed} / {side.total} moves reviewed</span>
      ...
      <p className="accuracy-method">Accuracy is the average of this app’s Stockfish-based move scores, including forced moves.</p>
      ...
      {summary.issues.length ? <ol className="review-issues">{summary.issues.map(issue => {
        ...
        return <li key={issue.beforePly}><button type="button" className="review-issue" aria-label={`Review ${move} · ${sideLabel(issue.color)} · ${issue.label}`} onClick={() => onInspect(issue.beforePly)}>
          <span className="issue-move"><strong>{move}</strong><small>{sideLabel(issue.color)}</small></span>
      ...
      {summary.issues.length > 0 && <p className="overview-hint">Select a move to compare it with the suggestions before it was played.</p>}
    </>}
  </section>;
}
