...
export function summarizeReview(nodes: readonly Pick<ReviewNode, 'fen' | 'moves'>[], qualities: readonly Quality[]) {
  ...
  return { sides, issues, total: sides.reduce((total, side) => total + side.total, 0), reviewed: sides.reduce((total, side) => total + side.reviewed, 0) };
}
